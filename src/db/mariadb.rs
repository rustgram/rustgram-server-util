...
pub async fn get_conn() -> Result<mysql_async::Conn, ServerCoreError>
{
...
*/
pub async fn exec<P>(sql: &str, params: P) -> Result<(), ServerCoreError>
where
	P: Into<Params> + Send,
{
	let mut conn = get_conn().await?;
...
*/
pub async fn exec_transaction<P>(data: Vec<TransactionData<'_, P>>) -> Result<(), ServerCoreError>
where
	P: Into<Params> + Send,
{
	let mut conn = get_conn().await?;
...
```
 */
pub async fn bulk_insert<F, P, T>(ignore: bool, table: &str, cols: &[&str], objects: Vec<T>, fun: F) -> Result<(), ServerCoreError>
where
	F: Fn(T) -> P,
	P: Into<Params>,
{
