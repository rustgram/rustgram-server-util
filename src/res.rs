...
impl IntoResponse<Response> for ServerCoreError
{
...
impl<T: Serialize> IntoResponse<Response> for JsonRes<T>
{
	fn into_response(self) -> Response
	{
		...
		let string = match json_to_string(&out) {
			Ok(s) => s,
			Err(e) => return Into::<ServerCoreError>::into(e).into_response(),
		};
...
pub type AppRes<T> = Result<T, ServerCoreError>;

pub type JRes<T> = Result<JsonRes<T>, ServerCoreError>;
